...
#ifdef __3DSLOADER_LOADWITH_OPENGL__
#ifdef WIN32
#include <windows.h>
#endif
#include <GL/gl.h>
...
CLoad3DS::CLoad3DS()
{
    ...
    #ifdef __3DSLOADER_LOADWITH_OPENGL__
    m_displaylist = -1;
    m_custom_material_enabled = false;
    #endif

}

CLoad3DS::CLoad3DS( const CLoad3DS &src)
{
    m_CurrentChunk = new tChunk;                // Initialize and allocate our current chunk
    m_TempChunk = new tChunk;                   // Initialize and allocate a temporary chunk
    #ifdef __3DSLOADER_LOADWITH_OPENGL__
...
bool CLoad3DS::Import3DS(char *strFileName)
{
    t3DModel *pModel = &m_model3D;
    ...
    if(!m_FilePointer) 
    {
        cout << "Unable to find the file: " << strFileName << "!" << endl;
        return false;
    }
    ...
    {
        cout << "Unable to load PRIMARY chuck from file: " << strFileName << "!" << endl;
        return false;
    }
...
void CLoad3DS::ReadUVCoordinates(t3DObject *pObject, tChunk *pPreviousChunk)
{
    ...
    pObject->pTexVerts = new C3dsLoaderVector2 [pObject->numTexVertex];
...
void CLoad3DS::ReadVertices(t3DObject *pObject, tChunk *pPreviousChunk)
{
    ...
    pObject->pVerts = new C3dsLoaderVector3 [pObject->numOfVerts];
    memset(pObject->pVerts, 0, sizeof(C3dsLoaderVector3) * pObject->numOfVerts);
...
C3dsLoaderVector3 Vector(C3dsLoaderVector3 vPoint1, C3dsLoaderVector3 vPoint2)
{
    C3dsLoaderVector3 vVector;                           // The variable to hold the resultant vector
...
C3dsLoaderVector3 AddVector(C3dsLoaderVector3 vVector1, C3dsLoaderVector3 vVector2)
{
    C3dsLoaderVector3 vResult;                           // The variable to hold the resultant vector
...
C3dsLoaderVector3 DivideVectorByScaler(C3dsLoaderVector3 vVector1, float Scaler)
{
    C3dsLoaderVector3 vResult;                           // The variable to hold the resultant vector
...
C3dsLoaderVector3 Cross(C3dsLoaderVector3 vVector1, C3dsLoaderVector3 vVector2)
{
    C3dsLoaderVector3 vCross;                                // The vector to hold the cross product
                                                // Get the X value
...
C3dsLoaderVector3 Normalize(C3dsLoaderVector3 vNormal)
{
...
{
    C3dsLoaderVector3 vVector1, vVector2, vNormal, vPoly[3];
    int i, j;
    ...
    for(int index = 0; index < pModel->numOfObjects; index++)
    {
        ...
        C3dsLoaderVector3 *pNormals      = new C3dsLoaderVector3 [pObject->numOfFaces];
        C3dsLoaderVector3 *pTempNormals  = new C3dsLoaderVector3 [pObject->numOfFaces];
        pObject->pNormals       = new C3dsLoaderVector3 [pObject->numOfVerts];
        ...
        C3dsLoaderVector3 vSum = {0.0, 0.0, 0.0};
        C3dsLoaderVector3 vZero = vSum;
        int shared=0;
...
void CLoad3DS::AssignMaterial(tLightMaterial &lightmaterial)
{
    m_lightmaterial = lightmaterial;
    m_custom_material_enabled = true;
}
...
void CLoad3DS::Render()
{
  if (m_displaylist == -1)
  {
    ...
    glNewList(m_displaylist, GL_COMPILE);
      ...
      if (m_custom_material_enabled)
      {
        glMaterialfv(GL_FRONT_AND_BACK,GL_DIFFUSE, m_lightmaterial.diffuse );
        glMaterialfv(GL_FRONT_AND_BACK,GL_AMBIENT, m_lightmaterial.ambiant );
        glMaterialfv(GL_FRONT_AND_BACK,GL_SPECULAR,m_lightmaterial.specular);
      }
      ...
      for(int i = 0; i < m_model3D.numOfObjects; i++)
      {
          // Make sure we have valid objects just in case. (size() is in the vector class)
          if(m_model3D.pObject.size() <= 0) break;

          // Get the current object that we are displaying
          t3DObject *pObject = &m_model3D.pObject[i];
              ...
              for(int j = 0; j < pObject->numOfFaces; j++)
              {
                  ...
                  for(int whichVertex = 0; whichVertex < 3; whichVertex++)
                  {
                      ...
                      if(pObject->bHasTexture) {
                          ...
                          if(m_model3D.pMaterials.size() && pObject->materialID >= 0) 
                          {
                              // Get and set the color that the object is, since it must not have a texture
                              BYTE *pColor = m_model3D.pMaterials[pObject->materialID].color;
...
#endif // __3DSLOADER_LOADWITH_OPENGL__
